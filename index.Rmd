...
output$num_displayed_points_on_map <- renderText({
  ...
  complete_data_count <- sitedat %>%
    filter(data_complete == 'YES') %>%
    summarize(count = n_distinct(MasterID)) %>%
    pull(count)
  
  missing_data_count <- sitedat %>%
...
output$map <- renderLeaflet({
  ...
  missing_data_default <- missing_data %>%
    filter(!StreamHealthIndex %in% c("Impacted and stressed", "Healthy, uncertain stress", "Impacted by unknown stress", "Healthy and potentially unstressed")) %>%
  ...
  if (input$show_missing_data) {
    map <- map %>%
      addMarkers(
        ...
        label = ~paste0(MasterID, ' (', yr, '): ', StreamHealthIndex, " - ", missing_status_text)
      ) %>%
      addMarkers(
