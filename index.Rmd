...
- R/sqi_dat_prep.R
- docs/app_figs.pptx
- docs/sqi_data_checking.xlsx
- data/cnstr.RData
- data/cntys.RData
- data/rwqbs.RData
- data/sheds.RData
- data/sqidat.RData
- data/sqidat_fordash.RData
- data/strms.RData
- .Renviron
---
...
.sidebar {
  ...
  resize: horizontal; /* Allows sidebar resizing */
  overflow: auto; /* Prevents content overflow */
}
.main-container {
  margin-left: 720px; /* Adjust based on sidebar width */
}
...
# data(sqidat) #this is sqi data from previous version of app
data(sheds)
data(cntys)
data(rwqbs)
data(cnstr)

# run dynamic data creation script
# this creates 'sqidat_fordash' dataset
source(here('R', 'sqi_dat_prep.R'))
# rename to sqidat to simplify
...
#       var %in% c('Cond', 'indexscore_cram', 'blc', 'bs', 'hy', 'ps'), 
...
cats <- reactive({
  req(nrow(sitedat()) > 0)
  req(input$year_select)  # Ensure year selection is available
  
  # Filter sitedat based on selected MasterID and selected year
  sitedat <- sitedat()
  out <- sitedat %>%
    filter(MasterID == selsit, yr == input$year_select)
...
output$num_displayed_points_on_map <- renderUI({
  
  # Separate the data based on the missing values
  sitedat <- sitedat()
  
  # Count unique MasterID for complete and missing data
  yes_data <- sitedat %>%
    filter(data_complete == 'YES') %>%
    distinct(MasterID, yr)
  
  yes_data_count_pairs <- nrow(yes_data)               # Count of unique (MasterID, yr) pairs
  yes_data_count_masterid <- n_distinct(yes_data$MasterID) # Count of unique MasterID
  
  # For 'NO' cases
  no_data <- sitedat %>%
    filter(data_complete == 'NO') %>%
    distinct(MasterID, yr)
  
  no_data_count_pairs <- nrow(no_data)                 # Count of unique (MasterID, yr) pairs
  no_data_count_masterid <- n_distinct(no_data$MasterID)   # Count of unique MasterID
  
  total_pairs <- yes_data_count_pairs + no_data_count_pairs
  num_unique_masterid <- n_distinct(sitedat$MasterID)
    
  # Display both counts with HTML line breaks
  HTML(glue("Number of site-visits with complete data: {yes_data_count_pairs} ({yes_data_count_masterid} unique sites)<br>
             Number of site-visits with incomplete data: {no_data_count_pairs} ({no_data_count_masterid} unique sites)<br>
             Total number of site-visits: {total_pairs} ({num_unique_masterid} unique sites)"))
...
output$map <- renderLeaflet({
  ...
  # Filter the complete and missing data
  complete_data <- sitedat %>%
    group_by(MasterID) %>%
    filter(all(data_complete == 'YES')) %>%
    ungroup()
    
  missing_data <- sitedat %>% 
    group_by(MasterID) %>%
    filter(any(data_complete == 'NO')) %>%
    ungroup() %>%
    mutate(
      missing_status_text = case_when(
        missing_chemistry == "YES" & missing_habitat == "YES" ~ "MISSING BOTH CHEMISTRY AND HABITAT",
        missing_chemistry == "YES" & missing_habitat == "NO" ~ "MISSING CHEMISTRY",
        missing_chemistry == "NO" & missing_habitat == "YES" ~ "MISSING HABITAT",
        TRUE ~ ""  # No missing information
      )
    ) %>%
  group_by(MasterID) %>%
  mutate(
    StreamHealthIndex_most_recent = StreamHealthIndex[which.max(yr)]
  ) %>%
  ungroup()
    
  # Define colors for each StreamHealthIndex category
  colors <- c(
    'Healthy and unstressed' = '#008000',        # Green
    'Healthy and resilient' = '#90EE90',          # LightGreen
    'Impacted by unknown stress' = '#FFB6C1',     # LightPink
    'Impacted and stressed' = '#DC143C',          # Crimson
    'Healthy, uncertain stress' = '#FFFF00',      # Yellow
    'Healthy and potentially unstressed' = '#008000' # Green
  )
  complete_data$color <- as.character(colors[complete_data$StreamHealthIndex])

  # Split missing data into separate data frames
  missing_data_impacted_stressed <- missing_data %>% 
    filter(StreamHealthIndex_most_recent == "Impacted and stressed") %>%
    mutate(icon = "impact-and-stressed.png")
  
  missing_data_healthy_uncertain_stress <- missing_data %>% 
    filter(StreamHealthIndex_most_recent == "Healthy, uncertain stress") %>%
    mutate(icon = "healthy-uncertain-stress.png")
  
  missing_data_impacted_unknown_stress <- missing_data %>% 
    filter(StreamHealthIndex_most_recent == "Impacted by unknown stress") %>%
    mutate(icon = "impact-by-unknown-stress.png")
  
  missing_data_healthy_unstressed <- missing_data %>% 
    filter(StreamHealthIndex_most_recent %in%  c("Healthy and potentially unstressed", "Healthy and unstressed")) %>%
    mutate(icon = "healthy-potentially-unstressed-icon.png")
  
  # Create default icon for any missing data that doesn't fit into specific categories
  missing_data_default <- missing_data %>%
    filter(
      !StreamHealthIndex_most_recent %in% c(
        "Impacted and stressed", 
        "Healthy, uncertain stress", 
        "Impacted by unknown stress", 
        "Healthy and potentially unstressed")
    ) %>%
    #mutate(icon = "default.png")
    mutate(icon = "healthy-potentially-unstressed-icon.png")
  
  # Function to create icons dynamically based on icon column
  make_icon <- function(icon) {
    makeIcon(
      iconUrl = paste0("https://nexus.sccwrp.org/smcchecker/download/icons-for-sqi/", icon),
      iconWidth = 10,
      iconHeight = 10
    )
  }
  ...
  legend_title <- HTML("
    <div>
      <strong>Shape:</strong> 
      <span style='font-size: 1.2em;'>&#9899;</span> = Complete Data, 
      <span style='font-size: 1.2em;'>&#9632;</span> = Missing Data
    </div>
    <div>
      <strong>Color:</strong>
    </div>
  ")
   ...
   # Render the map
  map <- mapview(polys, fill = FALSE, label = polys$pllb, homebutton = FALSE, popup = NULL, legend = FALSE) %>%
    .@map %>%
    
    # Add markers for complete data with color based on StreamHealthIndex
    addCircleMarkers(
      data = complete_data, 
      layerId = ~MasterID,
      stroke = TRUE,
      color = 'black',
      fill = TRUE,
      fillColor = ~color,   # Use the mapped color based on StreamHealthIndex,
      radius = 6, 
      weight = 1,
      fillOpacity = 1,
      label = ~paste0(MasterID, ' (', yr, '): ', StreamHealthIndex)
    )
    
  # Conditionally add missing data markers based on checkbox input
  if (input$show_missing_data) {
  #if (TRUE) {
    map <- map %>%
      addMarkers(
        data = missing_data_impacted_stressed,
        layerId = ~MasterID,
        icon = make_icon("impact-and-stressed.png"),
        label = ~paste0(MasterID, ' (', yr, '): ', StreamHealthIndex_most_recent, " - ", missing_status_text)
      ) %>%
      addMarkers(
        data = missing_data_healthy_uncertain_stress,
        layerId = ~MasterID,
        icon = make_icon("healthy-uncertain-stress.png"),
        label = ~paste0(MasterID, ' (', yr, '): ', StreamHealthIndex_most_recent, " - ", missing_status_text)
      ) %>%
      addMarkers(
        data = missing_data_impacted_unknown_stress,
        layerId = ~MasterID,
        icon = make_icon("impact-by-unknown-stress.png"),
        label = ~paste0(MasterID, ' (', yr, '): ', StreamHealthIndex_most_recent, " - ", missing_status_text)
      ) %>%
      addMarkers(
        data = missing_data_healthy_unstressed,
        layerId = ~MasterID,
        icon = make_icon("healthy-potentially-unstressed-icon.png"),
        label = ~paste0(MasterID, ' (', yr, '): ', StreamHealthIndex_most_recent, " - ", missing_status_text)
      )
    if (nrow(missing_data_default) > 0){
      map <- map %>%
        addMarkers(
          data = missing_data_default,
          layerId = ~MasterID,
          icon = make_icon("healthy-potentially-unstressed-icon.png"),
          label = ~paste0(MasterID, ' (', yr, '): ', StreamHealthIndex_most_recent, " - ", missing_status_text)
        )
  ...
  map %>%
    addLegend(
      title = legend_title,
      position = "bottomleft",
      colors = unname(colors),        # Pass only the colors
      labels = names(colors),         # Pass the labels
      opacity = 1
    )
})
...
observeEvent(input$map_marker_click, {
  ...
  point_clicked_id <<- input$map_marker_click$id
  
  # Get sitedat and filter by selected MasterID
  sitedat <- sitedat()
  years <- sitedat %>%
    filter(MasterID == point_clicked_id) %>%
    pull(yr) %>%
    unique() %>%
    sort(decreasing = TRUE)  # Sort years in ascending order
  
  # Update the year selectInput with available years for the selected MasterID
...
#     clicked_id <- click$id
#     
#     # Increase the radius of the clicked marker
#     leafletProxy("map") %>%
#       setRadius(layerId = clicked_id, radius = 10)
#     
#     # Reset the radius of the previously clicked marker if it's different
#     if (!is.null(previously_clicked()) && previously_clicked() != clicked_id) {
#       leafletProxy("map") %>%
#         setRadius(layerId = previously_clicked(), radius = 5)
#     }
#     
#     # Update the previously clicked marker ID
#     previously_clicked(clicked_id)
#   }
# })
...
output$overall <- flexdashboard::renderValueBox({
  ...
  colin <- ifelse(is.na(colin), "gray", colin)
...
output$plodstbio <- renderPlot({
  ...
  grid.arrange(grobs = dstplo$plos, nrow = 1)
}, height = 180)
...
output$plodsthab <- renderPlot({
  ...
  # patchwork
  #wrap_plots(unlist(dstplo), nrow = 1)
  grid.arrange(grobs = dstplo$plos, nrow = 1)
}, height = 180)
...
generate_pdf_report <- function(file) {
    ...
    # Summary Section
    filtered_by <- input$flby
    selected_filters <- paste(input$flts, collapse = ", ")

    unique_sites_num <- length(unique(tmp_dat$MasterID))
    unique_sample_num <- nrow(tmp_dat)
    
    initial_year <- min(tmp_dat$yr)
    final_year <- max(tmp_dat$yr)
    
    # Help func
    get_category_count <- function(data, groupby_col, category_name) {

      groupby_col <- rlang::sym(groupby_col)
      summary_data <- data %>%
        group_by(!!groupby_col) %>%
        summarise(
          distinct_count = n_distinct(MasterID), 
          row_count = n()
        )

      
      category_counts <- summary_data %>%
        filter(!!groupby_col == category_name) %>%
        summarise(
          distinct_count = first(distinct_count),
          row_count = first(row_count)
        ) %>%
        mutate(
          distinct_count = ifelse(is.na(distinct_count), 0, distinct_count),
          row_count = ifelse(is.na(row_count), 0, row_count)
        )
      
      
      # Handling cases where the specified category might not exist
      if (nrow(category_counts) == 0) {
        return(list(raw_site_count = 0, raw_sample_count = 0, pct_site_count = 0, pct_sample_count = 0))
      } else {
        return(list(
          raw_site_count = category_counts$distinct_count,
          raw_sample_count = category_counts$row_count,
          pct_site_count = round((category_counts$distinct_count / length(unique(data$MasterID))) * 100 , 1),
          pct_sample_count = round((category_counts$row_count  / nrow(data)) * 100, 1)
        ))
      }
    }
    
    # Duy: 1,2,3,etc. is the order of the category in the word file that Rafi sent
    # Overall condition section
    overall_1_count <- get_category_count(tmp_dat, 'SQI', 'Healthy and unstressed')
    overall_2_count <- get_category_count(tmp_dat, 'SQI', 'Healthy and resilient')
    overall_3_count <- get_category_count(tmp_dat, 'SQI', 'Impacted and stressed')
    overall_4_count <- get_category_count(tmp_dat, 'SQI', 'Impacted by unknown stress')
    
    
    # Biological condition section
    biological_1_count <- get_category_count(tmp_dat, 'biological_condition', 'Healthy')
    biological_2_count <- get_category_count(tmp_dat, 'biological_condition', 'Impacted for ASCI')
    biological_3_count <- get_category_count(tmp_dat, 'biological_condition', 'Impacted for CSCI')
    biological_4_count <- get_category_count(tmp_dat, 'biological_condition', 'Impacted for CSCI and ASCI')

    # Stress condition section
    stress_1_count <- get_category_count(tmp_dat, 'stress_condition', "Low stress")
    stress_2_count <- get_category_count(tmp_dat, 'stress_condition', "Stressed by chemistry degradation")
    stress_3_count <- get_category_count(tmp_dat, 'stress_condition', "Stressed by habitat degradation")
    stress_4_count <- get_category_count(tmp_dat, 'stress_condition', "Stressed by chemistry and habitat degradation")
    stress_5_count <- get_category_count(tmp_dat, 'stress_condition', "Stressed by low levels of chemistry or habitat degradation")
    ...
    # Wrap the selected_filters text to fit within a certain width
    wrapped_text <- strwrap(selected_filters, width = 80)  # Adjust the width as needed

    
    # Title and introduction
    text(left_margin, 0.95, "Summary Report", adj = 0, cex = 1.5)
    text(left_margin, 0.90, paste0("Stream quality in selected ", filtered_by, ": "), adj = 0, cex = 0.8)
    # Display each line of the wrapped text
    y_pos_start <- 0.85
    line_height <- 0.02  # Adjust the line height as needed
    for (line in wrapped_text) {
        text(left_margin, y_pos_start, line, adj = 0, cex = 0.8)
        y_pos_start <- y_pos_start - line_height
    }
    
    # Calculate the next starting y-position after the wrapped text
    next_y_pos <- y_pos_start - 0.02  # Additional space before the next section
    
    # Subsequent text, starting from the calculated position
    text(left_margin, next_y_pos, paste(unique_sites_num, "sites with complete data for all years and", unique_sample_num, "samples evaluated."), adj = 0, cex = 0.8)
    next_y_pos <- next_y_pos - line_height
    text(left_margin, next_y_pos, paste("Data spans from", initial_year, "to", final_year, "."), adj = 0, cex = 0.8)
    
    # Overall Condition Section
    text(left_margin, 0.7, "Overall Condition", adj = 0, cex = 1.2)
    text(left_margin, 0.65, paste0( overall_1_count$raw_sample_count, " of ", unique_sample_num, " samples (", overall_1_count$pct_sample_count ,"%)", " are healthy and unstressed"), adj = 0, cex = 0.8)
    text(left_margin, 0.625, paste0( overall_2_count$raw_sample_count, " of ", unique_sample_num, " samples (", overall_2_count$pct_sample_count ,"%)", " are healthy and resilient"), adj = 0, cex = 0.8)
    text(left_margin, 0.60, paste0( overall_3_count$raw_sample_count, " of ", unique_sample_num, " samples (", overall_3_count$pct_sample_count ,"%)", " are impacted and stressed"), adj = 0, cex = 0.8)
    text(left_margin, 0.575, paste0( overall_4_count$raw_sample_count, " of ", unique_sample_num, " samples (", overall_4_count$pct_sample_count ,"%)", " impacted by unknown stress"), adj = 0, cex = 0.8)
    
    
    
    
    # Biological Condition Section
    text(left_margin, 0.525, "Biological Condition", adj = 0, cex = 1.2)
    text(left_margin, 0.500, paste0(biological_1_count$raw_sample_count, " of ", unique_sample_num, " samples (", biological_1_count$pct_sample_count ,"%)", " are healthy"), adj = 0, cex = 0.8)
    text(left_margin, 0.475, paste0(biological_2_count$raw_sample_count, " of ", unique_sample_num, " samples (", biological_2_count$pct_sample_count ,"%)", " are impacted for the Algal Stream Condition Index (ASCI)"), adj = 0, cex = 0.8)
    text(left_margin, 0.450, paste0(biological_3_count$raw_sample_count, " of ", unique_sample_num, " samples (", biological_3_count$pct_sample_count ,"%)", " are impacted for the California Stream Condition Index (CSCI) for benthic macroinvertebrates"), adj = 0, cex = 0.8)
    text(left_margin, 0.425, paste0(biological_4_count$raw_sample_count, " of ", unique_sample_num, " samples (", biological_4_count$pct_sample_count ,"%)", " are impacted for both the ASCI and CSCI"), adj = 0, cex = 0.8)
    
    
    
    
    # Stress Condition Section
    text(left_margin, 0.375, "Stress Condition", adj = 0, cex = 1.2)
    text(left_margin, 0.350, paste0( stress_1_count$raw_sample_count, " of ", unique_sample_num, " samples (", stress_1_count$pct_sample_count ,"%)", " have low stress"), adj = 0, cex = 0.8)
    text(left_margin, 0.325, paste0( stress_2_count$raw_sample_count, " of ", unique_sample_num, " samples (", stress_2_count$pct_sample_count ,"%)", " are stressed degraded water chemistry"), adj = 0, cex = 0.8)
    text(left_margin, 0.300, paste0( stress_3_count$raw_sample_count, " of ", unique_sample_num, " samples (", stress_3_count$pct_sample_count ,"%)", " are stressed by degraded habitat"), adj = 0, cex = 0.8)
    text(left_margin, 0.275, paste0( stress_4_count$raw_sample_count, " of ", unique_sample_num, " samples (", stress_4_count$pct_sample_count ,"%)", " are stressed by degraded both water chemistry and habitat"), adj = 0, cex = 0.8)
    text(left_margin, 0.250, paste0( stress_5_count$raw_sample_count, " of ", unique_sample_num, " samples (", stress_5_count$pct_sample_count ,"%)", " are stressed by low levels of water chemistry and habitat degradation"), adj = 0, cex = 0.8)
...
output$dlreport <- downloadHandler(
        filename = function() {
            paste("stream_quality_report-", Sys.Date(), ".pdf", sep="")
        },
        content = function(file) {
            generate_pdf_report(file)
        }
)
...
get_smc_lu_means <- function() {
  pool::dbGetQuery(pool, "SELECT * FROM smc_lu_means")
}


output$dl_smc_lu_means <- downloadHandler(
  filename = function() {
    paste("smc_lu_means", Sys.Date(), ".csv", sep = "")
  },
  content = function(file) {
    data <- get_smc_lu_means()
...
output$missing_info <- renderText({
  # Retrieve the selected MasterID from the cats() reactive expression
  selected_masterid <- cats()$MasterID
  print("selected_masterid")
  print(selected_masterid)
  req(selected_masterid)
  # Filter the site data based on the selected MasterID
  filtered_dat <- sitedat() %>% filter(MasterID == selected_masterid)
  req
  # Check if there is any filtered data
  if(nrow(filtered_dat) == 0){
    return("No data available for the selected MasterID.")
  }
  
  # Generate missing information text for each year in filtered data
  missing_info_text <- filtered_dat %>%
    # Adjust the year label by subtracting 1, if necessary
    mutate(
      YearLabel = as.character(yr),
      # Determine missing information
      MissingInfo = case_when(
        missing_chemistry == "YES" & missing_habitat == "YES" ~ "Missing chemistry and missing habitat data",
        missing_chemistry == "YES" ~ "Missing chemistry data",
        missing_habitat == "YES" ~ "Missing habitat data",
        TRUE ~ "Data Complete"
      )
    ) %>%
    # Combine year and missing information into one line for each year
    mutate(Line = paste0(YearLabel, ": ", MissingInfo)) %>%
    # Extract the lines as a vector and collapse into a single text block
    pull(Line) %>%
    paste(collapse = " | ")
...
column(width = 12, 
  ...
  column(width = 6, 
    ...
    uiOutput("num_displayed_points_on_map"),
    checkboxInput("show_missing_data", "Include sites with missing data", value = TRUE)
  
  ),
  
  column(width = 6, 
    selectInput("year_select", "After clicking on a site, choose the year below: ", choices = NULL),
    renderText({
      ...
      if(nrow(cats()) > 0){
        ...
        # Get unique years and join them with commas
        years <- paste(unique(cats()$yr), collapse = ", ")
        
        # Get unique COMID (assuming there's only one, or you want to take the first if there are multiple)
        comid <- unique(cats()$COMID)[1]
        
        # Get unique coordinates (assuming they remain the same across rows)
        crds <- cats()$geometry
        crds <- round(crds[[1]][c(1, 2)], 4)
        
        # Append all to text output without repetitions
        txt <- paste0(txt, selsit, ' (Year ', years, ', COMID ', comid, ', ', crds[1], 'W/', crds[2], 'N)')
    ...
    tags$p("Below is the summary data status for all years. If there is a year with missing data, data were filled with the estimated values for that year.", style = "font-size: 14px;"),
    textOutput("missing_info")
...
This "dynamic" version of the SQI dashboard shows results for the most recent data that has been submitted to the SMC data portal. For the previous, static version, visit [here](https://sccwrp.shinyapps.io/sqi_shiny/).
...
```{r}
fluidRow(
  column(12, 'Click here to download a summary of sites with complete data:'),
  column(2, downloadButtonRmd('dlreport', ''))
)
```

```{r}
fluidRow(
  column(12, 'Click here to download estimates for missing values:'),
  column(2, downloadButtonRmd('dl_smc_lu_means', ''))
)
```
...
Below is a glossary of terms used in the SQI. Please see the overview tab for a general description of the foundation and main components of the SQI. Labels `like this` describe column names in the spreadsheet that can be downloaded from the overview tab. Note that variables in the downloaded data with the `_ptile` suffix indicate the percentile value of the variable at a site relative to all other sites in the file. Physical habitat metrics with the `_raw` suffix are raw metric values, whereas those with the`_score` suffix indicate scored values (where 0 indicates degraded conditions and 1 indicates intact conditions).
...
__Lower range of likely scores__, `lower`: The *10th* percentile of predicted scores, given the level of development in the watershed. See Beck et al. (2019; [https://doi.org/10.1086/705996](https://doi.org/10.1086/705996)).

__Median likely score__, `meds`: The *50th* percentile of predicted scores, given the level of development in the watershed. See Beck et al. (2019; [https://doi.org/10.1086/705996](https://doi.org/10.1086/705996)).

__Upper range of likely scores__, `upper`: The *90th* percentile of predicted scores, given the level of development in the watershed. See Beck et al. (2019; [https://doi.org/10.1086/705996](https://doi.org/10.1086/705996)).
