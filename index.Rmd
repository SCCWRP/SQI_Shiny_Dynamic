...
sitedat <- reactive({
  print("look")
  print(length(unique(sqidat$MasterID)))
  # input
  flts <- input$flts
  flby <- input$flby
  polys <- polys()
...
output$num_displayed_points_on_map <- renderText({
  ...
  complete_data_count <- length(unique(complete_data$MasterID))
  
  missing_data <- sitedat %>% 
    group_by(MasterID) %>%
    filter(any(data_complete == 'NO')) %>%
    ungroup()
  missing_data_count <- length(unique(missing_data$MasterID))
  
  # Display both counts
  glue("Number of sites with complete data: {complete_data_count}\nNumber of sites with missing chemistry or missing habitat data: {missing_data_count}")
})
...
#   
#   # Filter groups based on selection to reapply styling
#   sitedat_group1 <- sitedat %>% 
#     filter(missing_chemistry == 'NO', missing_habitat == 'NO')
#   sitedat_group2 <- sitedat %>% 
#     filter(!(missing_chemistry == 'NO' & missing_habitat == 'NO'))
#   
#   # Filter for selected site
#   selsitplo <- sitedat %>% filter(MasterID == selsit)
#   selsitplocol <- getdsccol(selsitplo[['toshw']])
#   names(selsitplocol) <- NULL
#   
#   # Clear markers and reapply styles for all markers, including selected one
#   # map <- map %>%
#   #   clearMarkers() %>%
#   #   
#   #   # Group 1 markers
...
generate_pdf_report <- function(file) {
    ...
    text(left_margin, next_y_pos, paste(unique_sites_num, "sites and", unique_sample_num, "samples evaluated."), adj = 0, cex = 0.8)
...
column(width = 12, 
  ...
  column(width = 6, 
    ...
    textOutput("num_displayed_points_on_map"),
