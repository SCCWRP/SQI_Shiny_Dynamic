...
output$num_displayed_points_on_map <- renderText({
  ...
  sitedat <- sitedat()
  complete_data_count <- nrow(sitedat %>% filter(missing_chemistry == 'NO', missing_habitat == 'NO'))
  missing_data_count <- nrow(sitedat %>% filter(!(missing_chemistry == 'NO' & missing_habitat == 'NO')))
...
output$map <- renderLeaflet({
  ...
  mapview(polys, fill = F, label = polys$pllb, homebutton = F, popup = NULL, legend = F) %>%
    ...
    addCircleMarkers(
      ...
      layerId = ~MasterID,
      stroke = TRUE,
      color = 'red',       # Different border color
      fill = TRUE,
      fillColor = cols,
      radius = 7,          # Slightly larger marker
      weight = 1,
...
output$plodsthab <- renderPlot({
  ...
  dstplo <- dsthab()
  print("look here")
  print(packageVersion("patchwork"))
  print(dstplo$plos)
  # patchwork
...
```{r}
fluidRow(
  column(12, 'Click here to download a summary of selected sites:'),
